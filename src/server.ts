// VERSÃO ULTRA-ESTÁVEL - 14/04/2026 17:15
...
app.post("/nfce/emitir/:orderId", async (req, res) => {
  ...
  try {
    ...
    // 3. XML (CONSTRUÇÃO PASSO A PASSO PARA EVITAR ERRO 225)
    ...
    ender.ele("xLgr").txt(norm(p.emitente.logradouro)).up().ele("nro").txt(norm(p.emitente.numero || "SN")).up()
         .ele("xBairro").txt(norm(p.emitente.bairro)).up().ele("cMun").txt("5212501").up().ele("xMun").txt("LUZIANIA").up()
    ...
    res.json({ autorizado: cStat === "100", status: cStat === "100" ? "AUTHORIZED" : "REJECTED", motivo: ret?.xMotivo || ret?.protNFe?.infProt?.xMotivo || "Erro", cStat });
