...
function extrairCertificadoEChave(buffer, senha) {
    try {
        ...
        if (!certBags.length || !keyBags.length)
            throw new Error("Certificado ou chave nao encontrada.");
...
function montarSoapAutorizacao(xmlAssinado) {
    ...
    if (!match)
        throw new Error("Erro ao extrair NFe para o SOAP");
...
async function enviarParaSefazGo(xmlAssinado, ambiente, certBuffer, senha) {
    ...
    const httpsAgent = new https_1.default.Agent({ pfx: certBuffer, passphrase: senha, rejectUnauthorized: false, minVersion: "TLSv1.2" });
