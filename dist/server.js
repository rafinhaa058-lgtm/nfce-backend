...
const pad = (v, size) => String(v).padStart(size, "0");
...
// ================= SEFAZ =================
async function enviar(xml, certBuffer, senha) {
    const agent = new https_1.default.Agent({
        pfx: certBuffer,
        passphrase: senha,
        rejectUnauthorized: false,
    });
    const res = await axios_1.default.post(SEFAZ_GO.homo, xml, {
        httpsAgent: agent,
...
app.post("/nfce/emitir/:id", async (req, res) => {
    try {
        ...
        const p12 = node_forge_1.default.pkcs12.pkcs12FromAsn1(node_forge_1.default.asn1.fromDer(certBuffer.toString("binary")), payload.certificado.senha);
        ...
        const certPem = node_forge_1.default.pki.certificateToPem(bags[0].cert);
        ...
        const retorno = await enviar(xmlAssinado, certBuffer, payload.certificado.senha);
        return res.json({
            sucesso: true,
            chave_acesso: chave,
            resposta: retorno,
